"""
CPortal Plugin — Self-service portal for hosting customers
NS: Apr 2026
...
def _get_portal_config():
    ...
    try:
        ...
        rp_enabled = False
    return {
        'portal_title': cfg.get('portal_title', 'CPortal'),
        ...
        'default_language': cfg.get('default_language', 'pt-BR'),
...
def _vm_snapshots():
    ...
    if request.method == 'GET':
        ...
        description = data.get('description', f'Created via CPortal by {username}')
...
def _portal_snapshot_policies():
    ...
    try:
        ...
        logging.exception('[cportal] snapshot-policies failed')
...
def _create_ct():
    ...
    if tenant_id:
        try:
            ...
            logging.exception('[cportal] quota check failed')
    ...
    try:
        ...
        logging.warning(f'[cportal] CT {new_vmid} created but ACL grant failed: {e}')
...
def _destroy_guest():
    ...
    try:
        ...
        logging.error(f'[cportal] destroyed {vm_type} {vmid_int} on {cluster_id} but '
...
# ============================================================
# CPortal (NosPanel fork of Client Portal) - additions
# Feature registry (C5), message i18n contract (C2) and
# language resolution (C3). Upstream client_portal logic above.
# ============================================================

PLUGIN_ID = 'cportal'
FEATURES_DIR = os.path.join(PLUGIN_DIR, 'features')

_CPORTAL_MSGS = {
    'en': {
        'feature.disabled': 'Feature not enabled.',
        'feature.missing_ui': 'Feature UI missing.',
        'feature.ui_error': 'Feature UI unavailable.',
    },
    'pt-BR': {
        'feature.disabled': 'Recurso nao habilitado.',
        'feature.missing_ui': 'Interface do recurso ausente.',
        'feature.ui_error': 'Interface do recurso indisponivel.',
    },
}

def _tr(code, fallback):
    """Resolve a CPortal message by the request user's language (C2 contract)."""
    try:
        lang = _resolve_language()
        table = _CPORTAL_MSGS.get(lang) or _CPORTAL_MSGS['en']
        return table.get(code, fallback)
    except Exception:
        return fallback

def _load_feature_manifests():
    """Scan plugins/cportal/features/*/manifest.json (CPortal extension registry)."""
    out = []
    if not os.path.isdir(FEATURES_DIR):
        return out
    try:
        for name in sorted(os.listdir(FEATURES_DIR)):
            mdir = os.path.join(FEATURES_DIR, name)
            ...
            try:
                ...
                logging.error('[CPortal] invalid feature manifest %s: %s', mpath, e)
    except Exception as e:
        logging.error('[CPortal] feature scan failed: %s', e)
    return out

def _enabled_features():
    """Enabled features: explicit ids in config.json 'features' win; when the
    list is empty, features declaring default_enabled:true are active."""
    cfg = _load_config()
    want = cfg.get('features', []) or []
    want = [w for w in want if isinstance(w, str)]
    out = []
    for m in _load_feature_manifests():
        fid = m.get('id')
        if fid in want or (not want and m.get('default_enabled', False)):
            out.append(m)
    return out

def _portal_features():
    """Public feature payload for the portal (id/name/entry) - no secrets."""
    out = []
    for m in _enabled_features():
        fid = m.get('id')
        out.append({
            'id': fid,
            'name': m.get('name', fid),
            ...
            'feature_ui': '/api/plugins/cportal/api/feature/%s/ui.js' % fid,
        })
    return out

def _resolve_language():
    """Resolve the request user's saved language, else the portal default."""
    try:
        username = request.session.get('user', '')
        if username:
...
def register(app):
    ...
    register_plugin_route('cportal', 'config', _get_portal_config)
    register_plugin_route('cportal', 'my-vms', _get_my_vms)
    register_plugin_route('cportal', 'ct/create-options', _ct_create_options)
    register_plugin_route('cportal', 'ct/create', _create_ct)
    register_plugin_route('cportal', 'vm/destroy-options', _destroy_options)
    register_plugin_route('cportal', 'vm/destroy', _destroy_guest)
    register_plugin_route('cportal', 'vm/power', _vm_power)
    register_plugin_route('cportal', 'vm/console', _vm_console)
    register_plugin_route('cportal', 'vm/snapshots', _vm_snapshots)
    register_plugin_route('cportal', 'vm/snapshot-rollback', _vm_snapshot_rollback)
    register_plugin_route('cportal', 'vm/snapshot-delete', _vm_snapshot_delete)
    register_plugin_route('cportal', 'account/change-password', _change_password)
    # ISO mount for portal customers
    register_plugin_route('cportal', 'vm/isos', _list_allowed_isos)
    register_plugin_route('cportal', 'vm/iso-mount', _mount_iso)
    register_plugin_route('cportal', 'vm/iso-unmount', _unmount_iso)
    ...
    register_plugin_route('cportal', 'snapshot-policies', _portal_snapshot_policies)


    # CPortal feature registry (C5): each enabled feature may ship a ui.js
    # served from /api/plugins/cportal/api/feature/<id>/ui.js.
    for feat in _enabled_features():
        fid = feat.get('id')
        ui_path = os.path.join(FEATURES_DIR, fid, 'ui.js') if fid else ''
        if fid and os.path.isfile(ui_path):
            def _feature_ui_handler(_fid=fid, _path=ui_path):
                from flask import Response
                try:
                    with open(_path, encoding='utf-8') as f:
                        return Response(f.read(), mimetype='application/javascript')
                except Exception as e:
                    logging.error('[CPortal] feature ui failed %s: %s', _fid, e)
                    return jsonify({'error': _tr('feature.ui_error', 'Feature UI unavailable.')}), 500
            register_plugin_route('cportal', 'feature/%s/ui.js' % fid, _feature_ui_handler)
            logging.info('[CPortal] feature registered: %s', fid)
    logging.info("[PLUGINS] CPortal plugin registered")
