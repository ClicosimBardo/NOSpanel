# -*- coding: utf-8 -*-
"""
NosPanel Flask App Factory - Layer 8
...
def download_static_files():
    ...
    print("=" * 60)
    print("NosPanel Static Files Downloader")
    print("=" * 60)
    print()
...
"""
    ...
    rfb_entry = '''// noVNC entry point for NosPanel offline mode
...
'''
    ...
    if failed == 0:
        ...
        print("  NosPanel can run fully offline now (including VNC console)")
    else:
...
def _tls_setup_failed(reason, path):
    ...
    raise SystemExit(
        ...
        "NosPanel behind a reverse proxy (the reverse_proxy setting), or set "
...
def _generate_self_signed(cert_file, key_file, domain, app_name):
    ...
    cert.get_subject().O = app_name or "NosPanel"
    cert.get_subject().OU = app_name or "NosPanel"
    cert.get_subject().CN = domain or app_name or "NosPanel"
...
def _resolve_ssl_context(reverse_proxy, domain='', app_name='NosPanel',
...
def main(debug_mode=False):
    """Main entry point - starts NosPanel server."""
    ...
    if not is_initialized():
        ...
        print("  No admin account exists yet — open the NosPanel URL")
    ...
    for cluster_id, cluster_data in config.items():
        ...
        if ctype == 'xcpng':
            ...
            manager.start()
            g.cluster_managers[cluster_id] = manager
            print(f"Started NosPanel manager for cluster: {cluster_data['name']}")
    ...
    app_name = server_settings.get('app_name', 'NosPanel')
    if reverse_proxy:
    ...
    print("Starting NosPanel with Flask development server")
...
def _start_gevent_server(app, bind_host, port, ssl_context, domain, workers, http_redirect_port=-1):
    ...
    print(f"Starting NosPanel with Gevent WSGIServer ({workers} greenlets)", flush=True)
