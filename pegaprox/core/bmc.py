...
Redfish opt-in phase), we SSH to the PVE node — access NosPanel already has for
...
nothing on the host (NosPanel never auto-enables the IPMI channel — the optional
...
_HW_CONSENT_TEXT = {
    'en': {
        ...
        'summary': 'NosPanel will read hardware health directly on each node through its local IPMI interface.',
        'points': [
            ...
            'If you enable installation, NosPanel will install the "ipmitool" package and may load the IPMI kernel modules on the node.',
            'On strictly hardened systems the local IPMI interface may be intentionally disabled under a least-functionality baseline. Enabling this here does not override that — where the interface is absent, NosPanel reports no data rather than re-enabling it.',
        ],
    ...
    'de': {
        ...
        'summary': 'NosPanel liest den Hardware-Zustand direkt auf jedem Node über dessen lokale IPMI-Schnittstelle.',
        'points': [
            ...
            'Wenn Sie die Installation aktivieren, installiert NosPanel das Paket „ipmitool“ und lädt ggf. die IPMI-Kernelmodule auf dem Node.',
            'Auf streng gehärteten Systemen kann die lokale IPMI-Schnittstelle bewusst im Rahmen einer Least-Functionality-Baseline deaktiviert sein. Die Aktivierung hier hebt das nicht auf — wo die Schnittstelle fehlt, meldet NosPanel keine Daten, statt sie wieder zu aktivieren.',
        ],
    ...
    'fr': {
        ...
        'summary': "NosPanel lira l'état du matériel directement sur chaque nœud via son interface IPMI locale.",
        'points': [
            ...
            "Si vous activez l'installation, NosPanel installera le paquet « ipmitool » et pourra charger les modules noyau IPMI sur le nœud.",
            "Sur les systèmes fortement durcis, l'interface IPMI locale peut être volontairement désactivée dans le cadre d'une base de moindre fonctionnalité. L'activer ici ne l'annule pas — là où l'interface est absente, NosPanel ne renvoie aucune donnée plutôt que de la réactiver.",
        ],
    ...
    'es': {
        ...
        'summary': 'NosPanel leerá el estado del hardware directamente en cada nodo a través de su interfaz IPMI local.',
        'points': [
            ...
            'Si activa la instalación, NosPanel instalará el paquete «ipmitool» y podrá cargar los módulos del kernel IPMI en el nodo.',
            'En sistemas muy reforzados, la interfaz IPMI local puede estar deshabilitada intencionadamente bajo una base de funcionalidad mínima. Activarla aquí no anula eso: donde la interfaz no existe, NosPanel no informa de ningún dato en lugar de reactivarla.',
        ],
    ...
    'pt': {
        ...
        'summary': 'O NosPanel lerá o estado do hardware diretamente em cada nó através da sua interface IPMI local.',
        'points': [
            ...
            'Se ativar a instalação, o NosPanel instalará o pacote «ipmitool» e poderá carregar os módulos de kernel IPMI no nó.',
            'Em sistemas fortemente reforçados, a interface IPMI local pode estar intencionalmente desativada sob uma base de funcionalidade mínima. Ativá-la aqui não anula isso — onde a interface não existe, o NosPanel não devolve dados em vez de a reativar.',
        ],
    ...
    'ko': {
        ...
        'summary': 'NosPanel는 각 노드의 로컬 IPMI 인터페이스를 통해 하드웨어 상태를 직접 읽습니다.',
        'points': [
            ...
            '설치를 활성화하면 NosPanel가 노드에 "ipmitool" 패키지를 설치하며, IPMI 커널 모듈을 로드할 수도 있습니다.',
            '강력하게 강화된 시스템에서는 최소 기능 기준에 따라 로컬 IPMI 인터페이스가 의도적으로 비활성화되어 있을 수 있습니다. 여기서 활성화하더라도 이를 무효화하지 않습니다. 인터페이스가 없는 경우 NosPanel는 이를 다시 활성화하지 않고 데이터를 보고하지 않습니다.',
        ],
    ...
    'it': {
        ...
        'summary': "NosPanel leggerà lo stato dell'hardware direttamente su ogni nodo tramite la sua interfaccia IPMI locale.",
        'points': [
            ...
            "Se abiliti l'installazione, NosPanel installerà il pacchetto « ipmitool » e potrà caricare i moduli kernel IPMI sul nodo.",
            "Su sistemi fortemente irrobustiti, l'interfaccia IPMI locale può essere disattivata intenzionalmente secondo una baseline di funzionalità minima. Abilitarla qui non annulla questo — dove l'interfaccia è assente, NosPanel non restituisce dati anziché riattivarla.",
        ],
...
_REDFISH_CONSENT_TEXT = {
    'en': {
        ...
        'summary': "NosPanel will read hardware health over the management network using the BMC Redfish API, with credentials you store per node.",
        'points': [
            "This is out-of-band and CREDENTIALED: NosPanel stores a BMC username and password per node and connects to the BMC over its management network — unlike in-band IPMI, which uses no credentials.",
            ...
            "BMC credentials are stored encrypted, but a compromise of NosPanel would expose management-plane access. Use a dedicated read-only BMC account where your BMC supports it.",
        ],
    ...
    'de': {
        ...
        'summary': "NosPanel liest den Hardware-Zustand über das Management-Netzwerk mittels BMC-Redfish-API, mit Zugangsdaten, die Sie pro Node speichern.",
        'points': [
            "Dies ist Out-of-Band und ZUGANGSDATEN-BASIERT: NosPanel speichert pro Node BMC-Benutzername und -Passwort und verbindet sich mit dem BMC über dessen Management-Netzwerk — anders als In-Band-IPMI, das keine Zugangsdaten nutzt.",
            ...
            "BMC-Zugangsdaten werden verschlüsselt gespeichert, doch eine Kompromittierung von NosPanel würde Zugriff auf die Management-Ebene offenlegen. Verwenden Sie nach Möglichkeit ein dediziertes, nur-lesendes BMC-Konto.",
        ],
    ...
    'fr': {
        ...
        'summary': "NosPanel lira l'état du matériel via le réseau de gestion à l'aide de l'API Redfish du BMC, avec des identifiants que vous stockez par nœud.",
        'points': [
            "Ceci est hors bande et BASÉ SUR DES IDENTIFIANTS : NosPanel stocke un nom d'utilisateur et un mot de passe BMC par nœud et se connecte au BMC via son réseau de gestion — contrairement à l'IPMI en bande, qui n'utilise aucun identifiant.",
            ...
            "Les identifiants BMC sont stockés chiffrés, mais une compromission de NosPanel exposerait l'accès au plan de gestion. Utilisez un compte BMC dédié en lecture seule lorsque votre BMC le permet.",
        ],
    ...
    'es': {
        ...
        'summary': "NosPanel leerá el estado del hardware a través de la red de gestión mediante la API Redfish del BMC, con credenciales que usted almacena por nodo.",
        'points': [
            "Esto es fuera de banda y BASADO EN CREDENCIALES: NosPanel almacena un usuario y una contraseña de BMC por nodo y se conecta al BMC a través de su red de gestión, a diferencia del IPMI en banda, que no usa credenciales.",
            ...
            "Las credenciales del BMC se almacenan cifradas, pero un compromiso de NosPanel expondría el acceso al plano de gestión. Use una cuenta de BMC dedicada de solo lectura cuando su BMC lo permita.",
        ],
    ...
    'pt': {
        ...
        'summary': "O NosPanel lerá o estado do hardware através da rede de gestão usando a API Redfish do BMC, com credenciais que você armazena por nó.",
        'points': [
            "Isto é fora de banda e BASEADO EM CREDENCIAIS: o NosPanel armazena um nome de utilizador e uma palavra-passe de BMC por nó e liga-se ao BMC através da sua rede de gestão, ao contrário do IPMI em banda, que não usa credenciais.",
            ...
            "As credenciais do BMC são armazenadas cifradas, mas um comprometimento do NosPanel exporia o acesso ao plano de gestão. Use uma conta de BMC dedicada só de leitura quando o seu BMC o permitir.",
        ],
    ...
    'ko': {
        ...
        'summary': "NosPanel는 노드별로 저장한 자격 증명으로 BMC Redfish API를 사용하여 관리 네트워크를 통해 하드웨어 상태를 읽습니다.",
        'points': [
            "이것은 대역 외이며 자격 증명 기반입니다. NosPanel는 노드별로 BMC 사용자 이름과 비밀번호를 저장하고 관리 네트워크를 통해 BMC에 연결합니다. 자격 증명을 사용하지 않는 대역 내 IPMI와 다릅니다.",
            ...
            "BMC 자격 증명은 암호화되어 저장되지만, NosPanel가 침해되면 관리 평면 접근이 노출됩니다. BMC가 지원하는 경우 전용 읽기 전용 BMC 계정을 사용하십시오.",
        ],
    ...
    'it': {
        ...
        'summary': "NosPanel leggerà lo stato dell'hardware tramite la rete di gestione usando l'API Redfish del BMC, con credenziali che memorizzi per ogni nodo.",
        'points': [
            "Questo è fuori banda e BASATO SU CREDENZIALI: NosPanel memorizza nome utente e password del BMC per ogni nodo e si connette al BMC tramite la sua rete di gestione, a differenza dell'IPMI in banda, che non usa credenziali.",
            ...
            "Le credenziali del BMC sono memorizzate cifrate, ma una compromissione di NosPanel esporrebbe l'accesso al piano di gestione. Usa un account BMC dedicato di sola lettura quando il tuo BMC lo consente.",
        ],
