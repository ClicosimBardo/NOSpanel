# -*- coding: utf-8 -*-
"""
NosPanel Alert Monitoring - Layer 7
...
def check_and_send_alerts():
    ...
    for alert in alerts_list:
        ...
        if triggered:
            ...
            subject = f"[NosPanel Alert] {alert_name}"
            body = f"""
Alert: {alert_name}
...
This is an automated alert from NosPanel.
"""
            ...
            _e = html_lib.escape
            html_body = f"""
<h2 style="color: #e74c3c;">⚠️ NosPanel Alert: {_e(str(alert_name))}</h2>
...
<p style="color: #666; font-size: 12px; margin-top: 20px;">This is an automated alert from NosPanel.</p>
"""
...
def check_update_available_alert():
    ...
    subject = f"[NosPanel] Update available — {latest}"
    body_lines = [
        f"A new NosPanel release is available: {latest}",
    ...
    html_body = (
        f"<h2>NosPanel update available</h2>"
...
def _emit_node_status_event(cluster_id, node, new_status, message, severity, recipients):
    ...
    if recipients:
        try:
            subject = f"[NosPanel] {alert_data['alert_name']}"
